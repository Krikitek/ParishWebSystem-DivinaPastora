...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function proceedToAcknowledgement() {
    ...
    displayReferenceNumber(referenceNumber)
  ...
  function displayReferenceNumber(refNumber) {
    const referenceSection = document.getElementById("reference-display")
    if (referenceSection) {
      const referenceNumberElement = referenceSection.querySelector(".reference-number")
  ...
  function initializePaymentPage() {
    // Update payment summary with new IDs
    document.getElementById("payment-reference-number").textContent = IntentionState.referenceNumber

    let totalAmount = 0
    let intentionsSummary = ""

    IntentionState.allIntentions.forEach((intention, index) => {
      totalAmount += intention.massOffering
      intentionsSummary += `${index + 1}. ${getIntentionTypeDisplay(intention.intentionType)} - PHP ${intention.massOffering.toFixed(2)}\n`
    })

    document.getElementById("payment-intentions-summary").textContent = intentionsSummary
    document.getElementById("payment-total-intentions").textContent = IntentionState.allIntentions.length
    document.getElementById("payment-total-amount").textContent = `₱${totalAmount.toFixed(2)}`
    document.getElementById("gcash-amount").value = totalAmount.toFixed(2)
    ...
    if (IntentionState.allIntentions.length > 0) {
      ...
      if (firstIntention.requesterCellphone) {
        document.getElementById("gcash-mobile").value = firstIntention.requesterCellphone
      }
    }
  }
  ...
  function showPaymentSuccess(transactionId, gcashNumber, amount, date) {
    ...
    // Update receipt details with new IDs
  ...
  window.editIntention = (index) => {
    ...
    // Update time field and name fields
    window.updateAvailableTimes()
    setTimeout(() => {
...
// Enhanced Mass Intention Form Handler with improved Step 3 & 4
document.addEventListener("DOMContentLoaded", () => {
  // State Management
  const IntentionState = {
    allIntentions: [],
  ...
  function updateProgressSteps(currentStep) {
    const steps = [
      { number: 1, filipino: "Detalye ng Intensyon", english: "Intention Details" },
      { number: 2, filipino: "Pagsusuri", english: "Review" },
      { number: 3, filipino: "Pagkilala", english: "Acknowledgement" },
      { number: 4, filipino: "Bayad", english: "Payment" },
    ]
